...
// extractInnerError extracts the inner error from NumError for comparison
func extractInnerError(err error) error {
...
// equalInnerError compares two errors by their inner error values
func equalInnerError(a, b error) bool {
	if a == nil {
		return b == nil
	}
	if b == nil {
	...
	innerA := extractInnerError(a)
	innerB := extractInnerError(b)
	// Compare error messages since sentinel errors may be different instances
	if innerA == nil {
		return innerB == nil
	}
	if innerB == nil {
	...
	return innerA.Error() == innerB.Error()
}

func TestStdlibParseUint32(t *testing.T) {
	for i := range parseUint32Tests {
		test := &parseUint32Tests[i]
		out, err := ParseUint(test.in, 10, 32)
		if uint64(test.out) != out || !equalInnerError(test.err, err) {
...
func TestStdlibParseUint64(t *testing.T) {
	for i := range parseUint64Tests {
		test := &parseUint64Tests[i]
...
func TestStdlibParseUint64Base(t *testing.T) {
	for i := range parseUint64BaseTests {
		test := &parseUint64BaseTests[i]
...
func TestStdlibParseInt32(t *testing.T) {
	for i := range parseInt32Tests {
		test := &parseInt32Tests[i]
...
func TestStdlibParseInt64(t *testing.T) {
	for i := range parseInt64Tests {
		test := &parseInt64Tests[i]
...
func TestStdlibParseInt64Base(t *testing.T) {
	for i := range parseInt64BaseTests {
		test := &parseInt64BaseTests[i]
...
func TestStdlibParseUint(t *testing.T) {
	switch IntSize {
	case 32:
		for i := range parseUint32Tests {
			test := &parseUint32Tests[i]
			...
			errForCompare := extractInnerError(err)
			testErrForCompare := extractInnerError(test.err)
			if uint64(test.out) != out || !reflect.DeepEqual(testErrForCompare, errForCompare) {
	...
	case 64:
		for i := range parseUint64Tests {
			...
			errForCompare := extractInnerError(err)
			testErrForCompare := extractInnerError(test.err)
			if test.out != out || !reflect.DeepEqual(testErrForCompare, errForCompare) {
				t.Errorf("ParseUint(%q, 10, 0) = %v, %v want %v, %v",
					test.in, out, err, test.out, test.err)
			}
		}
...
func TestStdlibAtoi(t *testing.T) {
	switch IntSize {
	case 32:
		for i := range parseInt32Tests {
			test := &parseInt32Tests[i]
			...
			var testErr error
			if test.err != nil {
				testErr = &NumError{"Atoi", test.in, extractInnerError(test.err)}
			}
...
func TestStdlibParseIntBitSize(t *testing.T) {
...
func TestStdlibParseUintBitSize(t *testing.T) {
	for i := range parseBitSizeTests {
		test := &parseBitSizeTests[i]
...
func TestStdlibParseIntBase(t *testing.T) {
...
func TestStdlibParseUintBase(t *testing.T) {
	for i := range parseBaseTests {
		test := &parseBaseTests[i]
		testErr := test.errStub("ParseUint", test.arg)
...
func TestStdlibNumError(t *testing.T) {
...
func TestStdlibNumErrorUnwrap(t *testing.T) {
...
func BenchmarkStdlibAtoi(b *testing.B) {
	b.Run("Pos", func(b *testing.B) {
		benchmarkAtoi(b, 1)
	})
	b.Run("Neg", func(b *testing.B) {
