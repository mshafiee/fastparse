...
func TestStdlibParseFloatPrefix(t *testing.T) {
	for i := range atoftests {
		test := &atoftests[i]
		if test.err != nil {
			continue
...
func testAtof(t *testing.T, opt bool) {
	...
	for i := 0; i < len(atoftests); i++ {
		...
		if numErr, ok := err.(*NumError); ok {
			errForCompare = numErr.Err
		}
		outs := FormatFloat(out, 'g', -1, 64)
		// Compare errors - extract inner error from test.err if it's a NumError
		testErrForCompare := test.err
		if testNumErr, ok := test.err.(*NumError); ok {
			testErrForCompare = testNumErr.Err
		}
		errMatch := (errForCompare == nil) == (testErrForCompare == nil)
		if errForCompare != nil && testErrForCompare != nil {
			// Compare the error values directly (they should be the same sentinel error)
			errMatch = errForCompare == testErrForCompare
		}
		if outs != test.out || !errMatch {
...
func TestStdlibAtof(t *testing.T) { testAtof(t, true) }

func TestStdlibAtofSlow(t *testing.T) { testAtof(t, false) }

func TestStdlibAtofRandom(t *testing.T) {
	initAtof()
...
func TestStdlibRoundTrip32(t *testing.T) {
	step := uint32(997)
	if testing.Short() {
		step = 99991
	}
...
func TestStdlibParseFloatIncorrectBitSize(t *testing.T) {
	const s = "1.5e308"
	const want = 1.5e308
...
func BenchmarkStdlibAtof64Decimal(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("33909", 64)
	}
}

func BenchmarkStdlibAtof64Float(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("339.7784", 64)
	}
}

func BenchmarkStdlibAtof64FloatExp(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("-5.09e75", 64)
	}
}

func BenchmarkStdlibAtof64Big(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("123456789123456789123456789", 64)
	}
}

func BenchmarkStdlibAtof64RandomBits(b *testing.B) {
	initAtof()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkStdlibAtof64RandomFloats(b *testing.B) {
	initAtof()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkStdlibAtof64RandomLongFloats(b *testing.B) {
	initAtof()
...
func BenchmarkStdlibAtof32Decimal(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("33909", 32)
	}
}

func BenchmarkStdlibAtof32Float(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("339.778", 32)
	}
}

func BenchmarkStdlibAtof32FloatExp(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ParseFloat("12.3456e32", 32)
	}
}

func BenchmarkStdlibAtof32Random(b *testing.B) {
	n := uint32(997)
...
func BenchmarkStdlibAtof32RandomLong(b *testing.B) {
	n := uint32(997)
	var float32strings [4096]string
	for i := range float32strings {
		n = (99991*n + 42) % (0xff << 23)
