...
check_first_digit:
	...
	// Parse integer part
	// Try AVX2 batch validation for longer strings
	MOVQ SI, BX
	SUBQ R8, BX              // BX = remaining bytes
	CMPQ BX, $16
	JGE try_avx2_digits
...
try_avx2_digits:
	...
	PMOVMSKB X3, AX
	TESTL AX, AX
	JNZ parse_int_loop       // Some byte < '0', use scalar
	...
	PMOVMSKB X4, AX
	TESTL AX, AX
	JNZ parse_int_loop       // Some byte > '9', use scalar
	
	// All 16 bytes are digits! But we still need scalar accumulation
	// because SIMD integer accumulation is complex
	// The validation just confirmed they're all digits
	// Fall through to scalar loop which will process them
	JMP parse_int_loop
