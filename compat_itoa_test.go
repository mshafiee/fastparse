...
func TestStdlibItoa(t *testing.T) {
...
func TestStdlibUitoa(t *testing.T) {
...
func TestStdlibFormatUintVarlen(t *testing.T) {
...
func BenchmarkStdlibAppendInt(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, test := range itob64tests {
			...
			stdlibBenchSink += len(dst)
		}
	}
}

func BenchmarkStdlibFormatUint(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for _, test := range uitob64tests {
			s := FormatUint(test.in, test.base)
			stdlibBenchSink += len(s)
...
func BenchmarkStdlibAppendUint(b *testing.B) {
	dst := make([]byte, 0, 30)
	for i := 0; i < b.N; i++ {
		for _, test := range uitob64tests {
...
func BenchmarkStdlibFormatIntSmall(b *testing.B) {
...
func BenchmarkStdlibAppendIntSmall(b *testing.B) {
	dst := make([]byte, 0, 30)
	const smallInt = 42
	for i := 0; i < b.N; i++ {
...
func BenchmarkStdlibAppendUintVarlen(b *testing.B) {
	for _, test := range varlenUints {
...
var stdlibBenchSink int // make sure compiler cannot optimize away benchmarks
