...
func TestParseUint32(t *testing.T) {
	for i := range parseUint32Tests {
		...
		if uint64(test.out) != out || !reflect.DeepEqual(test.err, errForCompare) {
...
func TestParseUint64(t *testing.T) {
	for i := range parseUint64Tests {
		...
		errForCompare := extractInnerError(err)
		if test.out != out || !reflect.DeepEqual(test.err, errForCompare) {
...
func TestParseInt32(t *testing.T) {
	for i := range parseInt32Tests {
		...
		errForCompare := extractInnerError(err)
		if int64(test.out) != out || !reflect.DeepEqual(test.err, errForCompare) {
...
func TestAtoi(t *testing.T) {
	...
	case 32:
		for i := range parseInt32Tests {
			...
			errForCompare := extractInnerError(err)
			if int(test.out) != out || !reflect.DeepEqual(test.err, errForCompare) {
	...
	case 64:
		for i := range parseInt64Tests {
			...
			out, err := Atoi(test.in)
			errForCompare := extractInnerError(err)
			if test.out != int64(out) || !reflect.DeepEqual(test.err, errForCompare) {
				t.Errorf("Atoi(%q) = %v, %v want %v, %v",
					test.in, out, err, test.out, test.err)
			}
		}
...
// TestNumError and TestNumErrorUnwrap are not needed in fastparse
// since we return simple errors directly
