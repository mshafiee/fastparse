...
func TestStdlibFtoa(t *testing.T) {
...
func TestStdlibFtoaPowersOfTwo(t *testing.T) {
...
func TestStdlibFtoaRandom(t *testing.T) {
	N := int(1e4)
	if testing.Short() {
		N = 100
	}
...
func TestStdlibFormatFloatInvalidBitSize(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
...
func BenchmarkStdlibFormatFloat(b *testing.B) {
...
func BenchmarkStdlibAppendFloat(b *testing.B) {
	dst := make([]byte, 30)
	for _, c := range ftoaBenches {
		b.Run(c.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
