...
func initAtofOnce() {
	// Fastparse returns simple errors directly, not wrapped in NumError
...
func testAtof(t *testing.T, opt bool) {
	...
	for i := 0; i < len(atoftests); i++ {
		...
		// Extract inner error from NumError for comparison (matches strconv behavior)
		errForCompare := err
		if numErr, ok := err.(*NumError); ok {
			errForCompare = numErr.Err
		}
		outs := FormatFloat(out, 'g', -1, 64)
		if outs != test.out || !reflect.DeepEqual(errForCompare, test.err) {
